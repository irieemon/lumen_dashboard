...
def _value_effort(x: float, y: float) -> Tuple[str, str]:
    """Return categorical value/effort strings for coordinates.

    Parameters
    ----------
    x, y:
        Coordinates on a 0-100 scale.
    """
...
def get_initiatives() -> pd.DataFrame:
    with _connect() as conn:
        query = (
